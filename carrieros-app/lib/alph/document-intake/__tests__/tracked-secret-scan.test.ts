...
test("tracked source passes the credential-pattern scan without exposing values",()=>{
  ...
  assert.match(result.stdout,/tracked paths and \d+ reachable historical blobs checked against 9 credential classes/);
...
test("tracked-secret scan covers high-risk provider and private-key classes",()=>{
  ...
  assert.match(scanner,/"rev-list","--objects","HEAD"/);
  assert.match(scanner,/cat-file","--batch-check=/);
  assert.match(scanner,/cat-file","blob",hash/);
  assert.match(scanner,/Matched values were not printed/);
  assert.match(scanner,/unreadable tracked file/);
  assert.match(scanner,/unreadable reachable history/);
});
