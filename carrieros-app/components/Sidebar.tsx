...
const groups = [
  ...
  { label: "Intelligence", items: [
    ...
    { name: "Nova AI", href: "/nova" }, { name: "ELD & Dashcam", href: "/integrations" },
...
export default function Sidebar() {
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-30 flex h-16 items-center justify-between border-b border-[#DDE5F0] bg-white px-4 lg:hidden">
      <Brand />
      <button ref={menuButton} type="button" aria-label="Open navigation" aria-controls="primary-sidebar" aria-expanded={open} onClick={() => setOpen(true)} className="rounded-xl border border-[#DDE5F0] px-3 py-2 text-sm font-semibold">Menu</button>
    </header>
    ...
    <aside id="primary-sidebar" aria-label="Primary navigation" className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-[#DDE5F0] bg-white transition-transform duration-200 lg:visible lg:translate-x-0 ${open ? "visible translate-x-0" : "invisible -translate-x-full"}`}>
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto px-4 pb-5" aria-label="Application modules">
        {groups.map((group) => { const labelId=`nav-${group.label.toLowerCase().replaceAll(/[^a-z]+/g,"-")}`; return <section key={group.label} aria-labelledby={labelId} className="mb-5">
          <p id={labelId} className="mb-1 px-3 text-[9px] font-semibold uppercase tracking-[.18em] text-[#94A3B8]">{group.label}</p>
        ...
        <Link href="/nova" aria-current={isActive(pathname,"/nova") ? "page" : undefined} onClick={() => setOpen(false)} className="block rounded-2xl border border-blue-100 bg-blue-50 p-4 hover:border-blue-300"><p className="text-sm font-semibold text-blue-900">Nova AI Partner</p><p className="mt-1 text-xs text-blue-700">Assistive and read-only.</p></Link>
      </nav>
    </aside>
  </>;
}
