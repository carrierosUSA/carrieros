...
type TrailerCurrentLoadTabProps = {
  loads: Load[];
};

export default function TrailerCurrentLoadTab({ loads }: TrailerCurrentLoadTabProps) {
  if (loads.length === 0) {
    return (
      <section className="rounded-[16px] bg-white p-8 text-center ring-1 ring-[#EAEAEA]">
        <p className="text-[15px] font-semibold text-slate-900">No current load</p>
        <p className="mt-1 text-[14px] text-slate-500">
          This trailer is not on an active dispatch.
        </p>
        <Link
          href="/loads"
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link
                ...
                className="inline-flex h-8 items-center rounded-full bg-[#F8FAFC] px-3 text-[12px] font-semibold text-slate-700 ring-1 ring-[#EAEAEA]"
              >
                Live
              </Link>
            </div>
