...
import { TRUCK_DOCUMENT_TYPE_LABELS } from "@/lib/types";
import type { TruckDocument, TruckDocumentType } from "@/lib/types";
import { CARRIEROS_COLORS } from "@/lib/design-system/colors";

type TruckDocumentsTabProps = {
  documents: TruckDocument[];
};

const documentTypes = Object.keys(
  TRUCK_DOCUMENT_TYPE_LABELS,
) as TruckDocumentType[];

function statusStyles(status: TruckDocument["status"]) {
  switch (status) {
    case "valid":
...
export default function TruckDocumentsTab({ documents }: TruckDocumentsTabProps) {
  const grouped = documentTypes.map((type) => ({
    type,
    label: TRUCK_DOCUMENT_TYPE_LABELS[type],
