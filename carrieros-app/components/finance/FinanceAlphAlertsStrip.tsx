...
import type { FinanceAlphAlert } from "@/lib/types/finance";
import { CARRIEROS_COLORS } from "@/lib/design-system/colors";

type FinanceAlphAlertsStripProps = {
  alerts: FinanceAlphAlert[];
  onFix: (alert: FinanceAlphAlert) => void;
  title?: string;
  subtitle?: string;
};

export default function FinanceAlphAlertsStrip({
  alerts,
  onFix,
  title,
  subtitle,
}: FinanceAlphAlertsStripProps) {
  if (alerts.length === 0) {
    return null;
  }
  ...
  return (
    <section
      aria-label="Alph finance alerts"
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        <div>
          <p className={`text-[14px] font-bold ${CARRIEROS_COLORS.warning.text}`}>
            {title ??
              `Alph found ${alerts.length} finance insight${alerts.length === 1 ? "" : "s"}`}
          </p>
          <p className="text-[13px] font-medium text-slate-600">
            {subtitle ??
              "Overdue invoices, cash flow, and payroll signals that need attention."}
          </p>
