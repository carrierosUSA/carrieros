...
const url=process.env.NEXT_PUBLIC_SUPABASE_URL;if(url&&!/^https:\/\/[a-z0-9-]+\.supabase\.co\/?$/i.test(url))failures.push("NEXT_PUBLIC_SUPABASE_URL must be an HTTPS Supabase project URL");
...
if((process.env.ALPH_DOCUMENT_BUCKET??"company-documents").trim().length<3)failures.push("ALPH_DOCUMENT_BUCKET is invalid");
